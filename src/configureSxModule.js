define([
	'fontoxml-dita/configureAsSimpletableTableElements',
	...
	'fontoxml-families/configureAsGroupWithBlock',
	'fontoxml-families/configureAsImageInFrame',
	'fontoxml-families/configureAsInlineAnchorToStructure',
	...
	'fontoxml-families/configureAsInlineObject',
	'fontoxml-families/configureAsInlineStructure',
	...
	return function configureSxModule (sxModule) {
		...
		configureAsInlineStructure(sxModule, 'self::alt and parent::image[fonto:in-inline-layout(.)]', t('alternative text'));
		...
		configureAsFrame(sxModule, 'self::dd and count(preceding-sibling::* | following-sibling::*) > 1', t('definition'), {
			defaultTextContainer: 'p',
			emptyElementPlaceholderText: t('type the definition'),
			showWhen: 'has-focus'
		});
		...
		// p in dd
		configureAsBlock(sxModule, 'self::p[parent::dd] and not(preceding-sibling::* or following-sibling::*)', t('paragraph'), {
			emptyElementPlaceholderText: t('type the definition')
		});
		...
		configureAsFrameWithBlock(sxModule, 'self::dt and count(preceding-sibling::* | following-sibling::*) > 1', t('term'), {
			showWhen: 'has-focus'
		});
		...
		configureContextualOperations(sxModule, 'self::fig[child::simpletable] and not(child::*[not(self::simpletable or self::title or self::desc)])', [
			{ name: ':fig-insert-title' },
			{ name: ':fig-insert-desc' },
			{ name: ':contextual-delete-fig' }
		]);
		...
		configureAsInlineAnchorToStructure(sxModule, 'self::fn and not(@conref) and fonto:in-inline-layout(.)', t('footnote'), {
			defaultTextContainer: 'p',
			emptyElementPlaceholderText: t('type the footnote'),
			endDelimiter: ']',
			startDelimiter: '[',
		...
		// p in fn
		configureAsBlock(sxModule, 'self::p[parent::fn] and not(preceding-sibling::* or following-sibling::*)', t('paragraph'), {
			emptyElementPlaceholderText: t('type the footnote')
		});
		...
		function getContextualOperationsForNoteType (noteType) {
			return NOTE_CONVERT_OPERATIONS
		...
		configureContextualOperations(sxModule, 'self::ol', [
		...
		configureProperties(sxModule, 'self::simpletable', {
			contextualOperations: [
				{ name: ':contextual-delete-simpletable' }
			],
		...
		configureContextualOperations(sxModule, 'self::ul', [
